...
public class JssorFlexImgGallery extends Composite {

	private MaterialCard materialCard;
	private static final String BASE_STYLE = "position:relative;margin:0 auto;top:0px;left:0px;width:800px;height:600px;overflow:hidden;visibility:hidden;border:1px solid #bbb;background-color:#eee;";
	private static final String BASE_STYLE_SLIDES = "cursor:default;position:relative;top:0px;right:1px;left:1px;width:800px;height:600px;overflow:hidden;";
	...
	public JssorFlexImgGallery() {
		...
		// areaSO4.setId(-15);
		//
		// final HTML sliderPanel = insertImages(
		...
		//
		// SimpleUploader fileUploadUi = new SimpleUploader("");
		//
		// MaterialCardAction cardAction = new MaterialCardAction();
		// cardAction.getElement().getStyle().setMarginTop(10, Unit.PX);
		// cardAction.add(fileUploadUi);
		// cardAction.setVisible(false);
		// materialCard.add(cardAction);

		initWidget(materialCard);

	}

	public JssorFlexImgGallery(List<CAreaSO> cAreaSOs) {

		Collections.sort(cAreaSOs, Comparator.comparing(CAreaSO::getIndex));

		final HTML sliderPanel = insertImages(cAreaSOs);
	...
	public void test() {
		...
		// areaSO4.setId(-15);
		//
		// final HTML sliderPanel = insertImages(Arrays.asList(areaSOw, areaSO002,
